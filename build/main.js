...
class Airquality extends utils.Adapter {
    ...
    async onReady() {
        ...
        //
        try {
            this.stationList = await (0, api_calls_1.getStations)();
        }
        catch (err) {
        ...
        //
        try {
            this.components = await (0, api_calls_1.getComponents)();
        }
        catch (err) {
    ...
    async worker() {
        ...
        try {
            for (const station of selectedStations) {
                ...
                const measurement = await (0, api_calls_1.getMeasurements)(station);
                if (measurement.success) {
                    await this.parseData(measurement);
                    success = measurement.success;
                }
                // only specified component
                const measurementComp = await (0, api_calls_1.getMeasurementsComp)(station, 2); // 2 = CO
    ...
    async parseData(payload) {
        ...
        */
        ...
        //
        await this.createObject(this.stationList[stationId].code, this.stationList[stationId].city, this.stationList[stationId].street);
        //
        for (const item of Object.keys(payload.measurementValues)) {
            const measurement = payload.measurementValues[item];
            await this.persistData(this.stationList[stationId].code, this.components[measurement[0]].code, this.components[measurement[0]].desc, measurement[1], // Value
